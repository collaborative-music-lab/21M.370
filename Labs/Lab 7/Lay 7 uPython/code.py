...
accel_filter = [utils.OneEuroFilter( min_cutoff=0.1, beta=0.2, d_cutoff=1.0) for _ in range(3)]
accel = [0,0,0]
gyro = [0,0,0]
...
while True:
    ...
    if (current_time - serial_timer) >= DATA_RATE/1000:
        ...
        status = imu.update()
        if status == False: print("I2C error")
        accel = imu.accel_cur
        # Scale values by 0.1 (1/10)
        gyro = [val * 256 for val in imu.gyro_cur]
